...
- シークレット保護: API レスポンス・履歴読み出しは履歴 JSONL の記録内容をそのまま返す。<code v-pre>{{env.X}}</code> で OS 環境変数から解決した値(長さ 4 文字以上)は履歴への書き込み時点で `***` にマスク済み(`src/core/history.ts` の `maskHistoryEntry` + `expandSecretVariants`。生の値だけでなく URL エンコード形も対象)のため、UI は履歴由来の値についてはマスク済みのものをそのまま表示するだけでよく、UI 側で追加のマスキングは行わない。ライブ実行結果(実行中のビュー、SSE 配信)は履歴経由ではないためマスク対象外

## セキュリティ(要件の制約の具体化)

1. **既定はループバックバインド**: `--host` 未指定時は `127.0.0.1` にのみバインドする(LAN 等外部からの接続は既定では受け付けない)。`--host` で明示的に非ループバック(`0.0.0.0` や LAN IP 等)を指定した場合のみ他ホストからの接続を受け付ける(docker-compose 等、コンテナ越しに UI を使いたいケースを想定した opt-in)
2. **起動時トークン**: サーバー起動時に `crypto.randomBytes` でトークンを生成し、`http://127.0.0.1:<port>/?token=<t>` をブラウザで開く。初回アクセスでトークンを検証して `SameSite=Strict` の Cookie に格納し、以降の API リクエストは Cookie + カスタムヘッダー(`X-Klaus-Token`)の二重チェック。トークン認証はバインド先ホストによらず常時維持する
3. **DNS rebinding 対策**: 全リクエストで `Host` ヘッダーを検証する。既定のループバックバインド(`127.0.0.1` / `localhost` / `::1`)時は `127.0.0.1:<port>` / `localhost:<port>` の allowlist と厳密照合し、不一致は 403。`--host` で明示的に非ループバックへバインドした場合は、接続元クライアントが送る Host ヘッダーのホスト名を事前に列挙できない(LAN IP 等、接続経路によって様々になり得る)ため、ポート一致のみへ検証を緩和する。この緩和は `--host` を明示指定した場合のみ有効な opt-in であり、既定の挙動(厳密な allowlist)は変えない
4. **CSRF 対策**: 状態変更 API(`POST` / `PUT` / `DELETE`)は Cookie 一致を必須にし、`Origin` ヘッダーが存在する場合は同一オリジンのみ許可する。オリジン判定は上記 3 の Host 判定と同じ基準(既定はループバック allowlist との厳密照合、`--host` 明示時のみポート一致に緩和)。Cookie 検証は緩和の対象外で、どちらの場合も常時維持する
...
- CLI の既定ポートは `4884`(`-p`/`--port` で変更可)。既定バインドホストは `127.0.0.1`(`-H`/`--host` で変更可)。エフェメラルポート(空きポート自動選択)は `startServer()` を port 未指定で直接呼び出した場合のみ使われる(CLI 経由では常に既定ポート)
