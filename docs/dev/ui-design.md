...
- シークレット保護: API レスポンス・履歴読み出しは履歴 JSONL の記録内容をそのまま返す。<code v-pre>{{env.X}}</code> で OS 環境変数から解決した値(長さ 4 文字以上)は履歴への書き込み時点で `***` にマスク済み(`src/core/history.ts` の `maskHistoryEntry`)のため、UI は履歴由来の値についてはマスク済みのものをそのまま表示するだけでよく、UI 側で追加のマスキングは行わない。ライブ実行結果(実行中のビュー)は履歴経由ではないためマスク対象外
