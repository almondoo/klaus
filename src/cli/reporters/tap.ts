 ...
 * 先に '\\' へエスケープしてから '#' を '\#' に変換することで、③④のエスケープ処理単体としては
 * 出力の '\' が「元から '\' だったもの」と「'#' のエスケープで付与したもの」のどちらに由来するか
 * 一意に判別できる(単射)。ただしパイプライン全体としてはこの限りではない: ②の sanitizeForTerminal が
 * 実際の改行1文字を `\` + `n` の2文字に変換するため、入力に元々含まれていたリテラル文字列 "\n" と
 * この時点で区別できなくなる。
 *
 * 出力ヘッダーは `TAP version 13`(下記 formatTap 参照)だが、この '\' / '#' エスケープは
 * TAP 13 の仕様には存在せず TAP 14 のものを採用している。意図的な選択で、`#` を含む description が
 * node-tap や Perl の TAP::Parser に SKIP/TODO ディレクティブと誤認されるのを防ぐ。代償として、
 * TAP 14 のエスケープを解さない Perl 系ハーネスでは '\' が二重に表示される。
 */
